...
function convertXMLToMarkdown(xml) {
  ...
  // --- Attachments ---
  $('ri\\:attachment').each((_, el) => {
    const filename = $(el).attr('ri:filename');
    $(el).replaceWith(`[${filename}](./${filename})`);
  });
  ...
  // --- Lists ---
  $('ul, ol').each((_, list) => {
    const bullet = list.tagName === 'ol' ? '1.' : '-';
    $(list).children('li').each((_, li) => {
      $(li).replaceWith(`${bullet} ${$(li).text()}\n`);
    });
    $(list).replaceWith('\n');
  });
